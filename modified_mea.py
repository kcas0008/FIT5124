...
from anti_mea import QueryMonitor, ExtractionDefenceWrapper, Watermark
...
def main(device='cpu'):
    ...
    train_loader, test_loader = get_dataloaders()          # we already need this
    ...
    distill_ds = build_extraction_dataset(target_protected, train_loader, device, args.temperature)
    ...
    # 7.4  Train student
    student = train_student(tr_ds, val_loader, target_protected, device,
                            T=args.temperature)

    # 7.5  Final metrics
    acc_t  = evaluate(target,  test_loader, device)
    acc_s  = evaluate(student, test_loader, device)
    fid    = functional_fidelity(student, target_protected, test_loader, device)
